...
2. Add a small dev backend/proxy path:
   ...
   - returns explicit `unavailable` while no provider is wired
...
5. Add server-side provider mode config:
   - default is `unavailable`
   - first real option is local vision through a local HTTP provider
   - production still uses a backend/proxy, not desktop keys

6. Add a local vision provider adapter:
   - sends screenshot payload + goal
   - asks for strict `GuidanceResult` JSON
   - returns provider name and errors clearly
   - completed on 2026-06-27
   - the `local-retired-local-vision-runtime` mode now calls retired local vision runtime's `/api/generate`
   - the request sends the screenshot payload through `images`
   - the prompt carries goal, display dimensions, scale factor, and calibration status
   - provider HTTP and parse failures return `unavailable`

7. Add response parsing and validation:
   - reject malformed JSON
   - reject invalid coordinates
   - never render failed provider output as a target
   - validate confidence, risk, confirmation, and target geometry
   - completed on 2026-06-27

Remaining before Phase 11:

8. Run one known-screen manual test:
   - open a known page
   - say a simple goal
   - send screenshot + goal
   - render one provider target
   - mark result useful or wrong
   - added `npm run guidance:known-screen` on 2026-06-27
   - runner posts a known screenshot + goal to the smoke endpoint
   - local provider verdict is pending because the retired local vision runtime endpoint was not reachable from this shell
9. Document provider errors, quota behavior, local model limits, and accuracy limits.
   - completed notes on 2026-06-27
   - current verdict: provider pipeline ready, target accuracy unproven
   - local provider was unavailable from this shell
   - if a reachable provider returns the wrong target, OCR/accessibility should move before Phase 11
10. Close or escalate:
   - close Phase 10.5 if one real target is useful enough
   - otherwise decide whether OCR/accessibility must happen before Phase 11
   - decision on 2026-06-27: close Phase 10.5 as provider pipeline ready, not accuracy proven
   - insert Phase 10.6 before Phase 11 for target accuracy and screen intelligence
...
Phase 10.5 proved the provider boundary, request shape, local adapter, validation, and known-screen runner. It did not prove that a model can choose the correct UI target. Safety guardrails depend on real tasks and real targets, so target accuracy should be handled before Phase 11.

Tasks:

1. Run the known-screen runner against a reachable local provider.
   - Step 1 result on 2026-06-27: provider readiness check exists.
   - Added `npm run guidance:provider:check`.
   - Step 2 result on 2026-06-27: local retired local vision runtime provider is ready outside the Codex sandbox.
   - Installed/started retired local vision runtime from the official macOS app path.
   - Pulled `llava:latest`.
   - `npm run guidance:provider:check` reports `[READY] local retired local vision runtime provider is reachable`.
   - Note: sandboxed local network checks can still fail even when retired local vision runtime is running.
2. Record the returned target as `useful`, `wrong`, or `unavailable`.
   - Step 3 result on 2026-06-27: `unavailable`.
   - Screenshot: `/tmp/toki-known-screen.png`.
   - Goal: `Click the message input box at the bottom.`
   - Provider reached: `local-retired-local-vision-runtime` / `llava:latest`.
   - Failure: provider output was rejected as an invalid `GuidanceResult`.
   - Step 4 result on 2026-06-27: raw provider output and validation issues are now printed.
   - Raw result: `llava:latest` returned a target with normalized `0..1` values (`x=0.389`, `y=0.781`, `width=0.520`, `height=0.412`) instead of CSS pixels.
   - Parser/validator decision: reject normalized target sizes instead of rendering them as a real target.
   - Step 5 result on 2026-06-27: candidate-assisted guidance returned a real target when the known-screen runner supplied `message-input` as a trusted UI candidate box.
   - Candidate result: `Message input box at 20,790 1430x60`.
   - Tradeoff: this proves candidate selection and anchoring, not automatic candidate discovery.
   - Step 6 result on 2026-06-27: the known-screen runner can now generate candidate evidence automatically from macOS Accessibility.
   - Tradeoff: this depends on macOS Accessibility permission and currently supports the known-screen runner, not the full live desktop guidance path.
   - Step 7 result on 2026-06-27: the macOS candidate probe can list visible apps and target a named app such as `Microsoft Edge`.
   - Live blocker: macOS denied `osascript` assistive access, so the probe resolved the app but could not read windows or candidates.
   - Step 8 result on 2026-06-27: Accessibility permission is now granted enough for Terminal candidates to work and for the Edge probe to select the real window process.
   - Live blocker: Microsoft Edge still returns zero candidates because `osascript` cannot read child elements from the Edge window (`Can't get object`).
   - Step 9 result on 2026-06-27: macOS Vision OCR candidate extraction works and returned 14 text candidates from `/tmp/toki-known-screen.png`.
   - Tradeoff: OCR can find visible text on browser-like screens, but it does not know whether text is a button, link, field, or paragraph.
   - Step 10 result on 2026-06-27: OCR-backed known-screen guidance returned a useful real provider target.
   - OCR result: `> Find and fix a bug in @filename at 9,809 235x17`.
   - Step 11 result on 2026-06-27: live desktop guidance requests now include macOS Vision OCR candidates when running the real provider path.
   - Runtime result: the app calls `collect_screen_candidates`, attaches candidate boxes to `GuidanceRequest.screen`, and shows candidate source/count/error in Debug.
   - Tradeoff: the live path currently shells through Swift/Vision for OCR, which is fine for target-accuracy smoke testing but may need a native bridge or persistent worker for production latency.
   - Step 12 result on 2026-06-27: live desktop real-guidance smoke now reaches the local provider from the running app.
   - Runtime result: the QA-only auto-smoke path captures the live screen, collects OCR candidates, downscales the screenshot, and posts a real provider request.
   - Tradeoff: the first live provider target was `USER`, so the pipeline works but target usefulness is not yet strong enough for Phase 11.
   - Step 13 result on 2026-06-27: native cursor tracking is implemented for macOS.
   - Problem: Tauri/WebView cursor coordinates are not reliable enough for the always-on, click-through overlay; the puck can park far from the actual cursor on macOS.
   - Implementation: Rust exposes `native_cursor_position`, macOS reads the cursor through CoreGraphics, and React uses that command before falling back to Tauri `cursorPosition()`.
   - Tradeoff: Windows/Linux still use fallback behavior until their native cursor adapters are added.
   - Acceptance still requires visual testing: the puck should stay close to the real cursor in the center, at edges, near the menu bar, and near the Dock.
3. If screenshot-only output is useful, close the smoke target gate.
4. If screenshot-only output is wrong, add OCR/accessibility evidence.
5. Build a UI candidate map:
   - visible text
   - accessibility labels
   - bounding boxes
   - role/type where available
6. Update the provider prompt to choose from candidates instead of raw pixels only.
7. Re-run the known-screen test.
8. Record the miss type and accuracy notes.
9. Replace fragile WebView cursor polling with native cursor tracking for the cursor-first overlay.
10. Move to Phase 11 only after at least one known-screen target is useful or after explicitly accepting the remaining accuracy risk.

Done when:

- one known-screen target is useful, or
- OCR/accessibility is explicitly chosen as the next blocker before safety

---

## Phase 10.7: Browser And Provider Accuracy Upgrade

Goal: improve real target accuracy before Phase 11 by testing stronger development providers and improving browser screen evidence.

Reason:

Phase 10.6 proved that the live guidance pipeline can capture the screen, collect OCR candidates, send a provider request, validate output, and render a target. It also proved that local retired local vision runtime is not accurate enough by itself for browser-heavy workflows. Browser UIs are the main product surface, and current macOS Accessibility extraction is weak for Edge/Chromium content. Phase 10.7 keeps this as a development accuracy track before safety policy work.

Steps:

1. Add `freellmapi-dev` provider mode.
   - Use FreeLLMAPI only as a development provider aggregator.
   - Keep production provider planning separate.
   - Preserve `unavailable` as the safe default.
2. Run known-screen tests through FreeLLMAPI vision models.
3. Compare target accuracy against local retired local vision runtime.
4. Add browser candidate strategy plan:
   - short term: OCR plus layout heuristics
   - mid term: native macOS AX bridge
   - long term: browser extension companion
5. Add a candidate ranking layer before provider calls.
6. Run browser known-screen tests on Edge/Chrome.
7. Record which provider/candidate path is reliable enough before Phase 11.

Done when:

- FreeLLMAPI can be selected as a dev provider, or the blocker is clearly recorded.
- at least one browser known-screen path has a useful/wrong verdict.
- the next accuracy path is chosen before Phase 11 Safety.
